            ...
            <%@ include file="/WEB-INF/views/common/header.jsp" %>
                <div id="app">
                    <div class="admin-container">
                        ...
                        <div class="top-area">
                            <button class="btn btn-primary" @click="openModal"><i class="fa-solid fa-plus"></i> 쿠폰
                                생성</button>
                        </div>
                        ...
                        <table class="coupon-table">
                            ...
                            <tbody>
                                <tr v-for="item in list" :key="item.couponNo || item.COUPON_NO">
                                    ...
                                    <td>
                                        <span v-if="(item.discountType || item.DISCOUNT_TYPE) == 'F'">{{
                                            Number(item.discountAmount || item.DISCOUNT_AMOUNT).toLocaleString()
                                            }}원</span>
                                        <span v-else>{{ item.discountAmount || item.DISCOUNT_AMOUNT }}%</span>
                                    </td>
                                    <td>{{ item.expireDays || item.EXPIRE_DAYS }}일</td>
                                    <td :title="item.notiMessage || item.NOTI_MESSAGE"
                                    ...
                                    <td>
                                        <button class="btn btn-info btn-sm"
                                            @click="fnIssue(item.couponNo || item.COUPON_NO)" title="전체 회원 발급">
                                            <i class="fa-regular fa-paper-plane"></i> 발급
                                        </button>
                                        <button class="btn btn-danger btn-sm"
                                            @click="fnDelete(item.couponNo || item.COUPON_NO)" style="margin-left: 5px;"
                                            title="삭제">
                        ...
                        <div class="modal-container">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    <label>쿠폰 이름</label>
                                    <input type="text" v-model="coupon.couponName" class="form-control"
                                ...
                                <div class="form-group">
                                    <label>알림 메시지</label>
                                    <textarea v-model="coupon.notiMessage" class="form-control" rows="3"
                                        placeholder="[선물] 고객님께 쿠폰이 도착했습니다!"></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button class="btn btn-primary" @click="fnCreate">생성</button>
                                <button class="btn btn-danger" @click="closeModal"
                                    style="background-color: #999;">취소</button>
                            </div>
                        </div>
                    </div>
                            ...
                            methods: {
                                ...
                                fnCreate() {
                                    if (!this.coupon.couponName) {
                                        Swal.fire('⚠️', '쿠폰명을 입력하세요.', 'warning');
                                        return;
                                    }
                                ...
                                fnIssue(couponNo) {
                                    Swal.fire({
                                        icon: 'warning',
                                        title: '⚠️',
                                        text: '정말 이 쿠폰을 모든 회원에게 발급하시겠습니까? (알림이 전송됩니다)',
                                        showCancelButton: true,
                                            ...
                                            success: (res) => {
                                                if (res.result == "success") {
                                                    Swal.fire({
                                                        icon: 'success',
                                                        title: '✅',
                                                        text: res.count + "명에게 발급 및 알림 전송이 완료되었습니다.",
                                                        confirmButtonColor: '#5dbb63'
                                                    });
