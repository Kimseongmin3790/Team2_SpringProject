                ...
                <%@ include file="/WEB-INF/views/common/header.jsp" %>
                    ...
                    <main class="content">
                        ...
                        <div class="tab-container">
                            <div class="tab-nav">
                                <button class="tab-button" :class="{ active: activeTab === 'dashboard' }"
                                    @click="activeTab = 'dashboard'">
                                    대시보드
                                </button>
                                <button class="tab-button" :class="{ active: activeTab === 'settlement' }"
                                    @click="activeTab = 'settlement'">
                                    매출 관리
                                </button>
                                <button class="tab-button" :class="{ active: activeTab === 'farm' }"
                                ...
                                <button class="tab-button" :class="{ active: activeTab === 'chats' }"
                                    @click="activeTab='chats'; loadChatRooms(); console.log('tab->chats')">
                                    채팅
                                </button>
                            </div>
                            ...
                            <div v-show="activeTab === 'dashboard'" class="tab-content">
                                ...
                                <div class="stats-grid">
                                    <div class="stat-card">
                                        ...
                                        <div class="stat-value">{{ stats.todayOrders }}<span class="stat-unit">건</span>
                                        </div>
                                    </div>
                                    <div class="stat-card">
                                        <div class="stat-label">오늘 매출</div>
                                        <div class="stat-value">{{ formatPrice(stats.todaySales) }}<span
                                                class="stat-unit">원</span></div>
                                    </div>
                                    <div class="stat-card">
                                        <div class="stat-label">등록 상품</div>
                                        <div class="stat-value">{{ stats.totalProducts }}<span
                                                class="stat-unit">개</span></div>
                                    </div>
                                    <div class="stat-card">
                                        <div class="stat-label">평균 평점</div>
                                        <div class="stat-value">{{ stats.avgRating }}<span class="stat-unit">/
                                                5.0</span></div>
                                    </div>
                                    ...
                                    <tbody>
                                        <tr v-for="order in recentOrders" :key="order.orderNo">
                                            ...
                                            <td>
                                                ...
                                                <span v-if="order.primaryRefundStatus" class="status-badge"
                                                    :class="getRefundStatusBadgeClass(order.primaryRefundStatus)">
                            ...
                            <div v-show="activeTab === 'settlement'" class="tab-content">
                                ...
                                <div class="sales-summary-card">
                                    <div class="summary-item">
                                        ...
                                        <div class="summary-value">{{ salesSummary.totalOrderCountSum }}<span
                                                class="summary-unit">건</span></div>
                                    </div>
                                    <div class="summary-item">
                                        <div class="summary-label">총 매출</div>
                                        <div class="summary-value">{{ formatPrice(salesSummary.totalSalesSum) }}<span
                                                class="summary-unit">원</span></div>
                                    </div>
                                    <div class="summary-item">
                                        <div class="summary-label">총 플랫폼 수수료</div>
                                        <div class="summary-value">{{ formatPrice(salesSummary.totalPlatformFeeSum)
                                ...
                                <div v-if="salesPeriod.type === 'monthly' && totalPages > 1" class="pagination">
                                    <button @click="changePage(salesPagination.currentPage - 1)"
                                        :disabled="salesPagination.currentPage <= 1" class="page-btn"> 이전 </button>
                                    <span v-for="page in totalPages" :key="page">
                                        <button @click="changePage(page)"
                                            :class="{ 'page-btn': true, 'active': salesPagination.currentPage === page }">
                                            {{ page }} </button>
                                    </span>
                                    <button @click="changePage(salesPagination.currentPage + 1)"
                                        :disabled="salesPagination.currentPage >= totalPages" class="page-btn"> 다음
                                    </button>
                                </div>
                            </div>
                            ...
                            <div v-show="activeTab === 'farm'" class="tab-content">
                                ...
                                <form @submit.prevent="updateFarmInfo">
                                    ...
                                    <button type="submit" class="btn btn-primary" style="margin-top: 2rem;">농가 정보
                                        저장</button>
                                </form>
                            </div>
                            ...
                            <div v-show="activeTab === 'reviews'" class="tab-content">
                                ...
                                <div v-for="review in reviews" :key="review.id" class="review-card">
                                    ...
                                    <div v-if="review.reviewImages && review.reviewImages.length > 0"
                                        class="review-images-container">
                                        <img v-for="(imageUrl, index) in review.reviewImages" :key="index"
                                            :src="imageUrl" alt="리뷰 이미지" class="review-image-thumbnail">
                                    </div>
                                    <div v-if="review.comments && review.comments.length > 0"
                                        class="seller-reply-container">
                                        <div v-for="comment in review.comments" :key="comment.commentNo"
                                            class="seller-reply-item">
                                            <div class="seller-reply-body">
                                                <p class="seller-reply-author">{{ comment.userId }} (판매자)님의 답글:</p>
                                                <p v-if="editingCommentNo !== comment.commentNo"
                                                    class="seller-reply-content">{{ comment.contents}}</p>
                                                <textarea v-else v-model="comment.contents"
                                                    class="form-textarea seller-reply-edit-input"></textarea>
                                            ...
                                            <div class="seller-reply-actions">
                                                ...
                                                <template v-if="editingCommentNo !== comment.commentNo">
                                                    <button class="btn btn-secondary btn-sm"
                                                        @click="editComment(comment.commentNo)">수정</button>
                                                    <button class="btn btn-secondary btn-sm"
                                                        @click="deleteComment(comment.commentNo)">삭제</button>
                                                </template>
                                                <template v-else>
                                                    <button class="btn btn-primary btn-sm"
                                                        @click="saveEditedComment(comment)">저장</button>
                                                    <button class="btn btn-secondary btn-sm"
                                    ...
                                    <div class="review-meta">
                                        ...
                                        <button class="btn btn-secondary" @click="replyToReview(review.reviewNo)">답글
                                            작성</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-show="activeTab === 'chats'" class="tab-content">
                                <h2 style="margin-bottom: 1.5rem; color: #111827;">채팅</h2>
                                ...
                                <table v-else class="orders-table">
                                    ...
                                    <tbody>
                                        <tr v-for="(r, idx) in chatRooms" :key="r.roomId">
                                            <td>{{ idx + 1 }}</td>
                                            <td>{{ r.pName }}</td>
                                            <td>{{ r.buyerId }}</td>
                                            <td>{{ formatChatTime(r.lastMessageAt || r.udatetime || r.cdatetime) }}</td>
                                            <td class="text-center">
                                                <button class="btn btn-primary btn-sm" @click="openChat(r)">열기
                                                </button>
                                            </td>
                                        </tr>
                                    </tbody>
            ...
            const app = Vue.createApp({
                ...
                methods: {
                    ...
                    loadChatRooms: function () {
                        ...
                        $.ajax({
                            url: "${pageContext.request.contextPath}/chat/rooms.dox",
                            dataType: "json",
                            type: "POST",
                            success: function (data) {
                    ...
                    openChat(r) {
                        ...
                        window.open(
                            "${pageContext.request.contextPath}/chat.do?" + params.toString(),
                            "chatPopup",
