                ...
                <script>
                    ...
                    const app = Vue.createApp({
                        ...
                        methods: {
                                ...
                                $.ajax({
                                    ...
                                    success: (data) => {
                                        if (data.result === "success") {
                                            ...
                                            // 페이지가 비었는데 총개수는 남아있으면 이전 페이지로
                                            if (this.list.length === 0 && this.totalCount > 0 && this.currentPage > 1) {
                                                this.fnList(this.currentPage - 1);
                                            }
                                        } else {
                                            swError(data.message || "목록 조회 실패");
                                        }
                                    },
                                    error: () => swError("서버 오류가 발생했습니다.")
                                });
                            },
                            ...
                            async fnDeleteSelected() {
                                if (this.selectedItems.length === 0) {
                                    swWarn("삭제할 상품을 선택해주세요.");
                                    return;
                                }

                                const result = await Swal.fire({
                                    title: '⚠️',
                                    text: this.selectedItems.length + "개의 상품을 찜 목록에서 삭제하시겠습니까?",
                                    icon: 'warning',
                                ...
                                $.ajax({
                                    ...
                                    dataType: "json",
                                    data: {
                                        selectItem: JSON.stringify(this.selectedItems)
                                    },
                                    success: async (data) => {
                                        if (data.result === "success") {
                                            await swSuccess("선택한 상품이 삭제되었습니다.");
                                            this.selectedItems = [];
                                            this.fnList(this.currentPage);
                                        } else {
                                            swError("삭제 처리 중 오류가 발생했습니다: " + (data.message || ""));
                                        }
                                    },
                                    error: () => swError("서버 오류가 발생했습니다.")
                                });
                            ...
                            // 개별 삭제
                            async fnRemove(productNo) {
                                const result = await Swal.fire({
                                    title: '⚠️',
                                    text: "이 상품을 찜 목록에서 삭제하시겠습니까?",
                                    icon: 'warning',
                                    showCancelButton: true,
                                    confirmButtonText: '삭제',
                                    cancelButtonText: '취소',
                                ...
                                $.ajax({
                                    ...
                                    success: async (data) => {
                                        if (data.result === "success") {
                                            await swSuccess("삭제되었습니다.");
                                            this.selectedItems = [];
                                            this.fnList(this.currentPage);
                                        } else {
                                            swError(data.message || "삭제 실패");
                                        }
                                    },
                                    error: () => swError("서버 오류가 발생했습니다.")
                                });
