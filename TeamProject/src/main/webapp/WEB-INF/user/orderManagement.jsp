<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
                        ...
                        <div class="main-container">
                                ...
                                <div class="filter-grid">
                                    ...
                                    <div class="filter-item search-col">
                                        ...
                                        <div class="search-wrapper">
                                            ...
                                            <input type="text" class="search-input" placeholder="주문번호, 구매자명, 상품명 검색"
                                                v-model="searchQuery">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="table-section">
                                <div class="table-wrapper">
                                    <table>
                                        <thead>
                                            <tr>
                                                <th style="width: 3rem;"><input type="checkbox"
                                                        @change="toggleSelectAll" :checked="isAllSelected"></th>
                                                <th>주문번호</th>
                                                <th>주문일시</th>
                                                <th>상품명</th>
                                                <th>구매자</th>
                                        ...
                                        <tbody>
                                            <tr v-for="order in orders" :key="order.orderNo">
                                                <td><input type="checkbox" :value="order.orderNo"
                                                        v-model="selectedOrders"></td>
                                                <td>
                                                    <a href="#" class="order-no-link"
                                                        @click.prevent="openDetailModal(order)">
                                                        {{ order.orderNo }}
                                                    </a>
                                                </td>
                                                ...
                                                <td class="text-center" style="vertical-align: middle;">
                                                    ...
                                                    <div v-else class="action-controls">
                                                        <select class="status-select" :value="order.status"
                                                            ...
                                                            <option
                                                                v-for="option in getValidStatusOptions(order.status)"
                                                                :key="option" :value="option">
                                                                {{ option }}
                                                            </option>
                                                        </select>
                                                        <button v-if="order.status === '배송 준비중'" class="btn"
                            ...
                            <div class="pagination-container" v-if="totalPages > 0">
                                ...
                                <span v-for="page in pageNumbers" :key="page">
                                    <button class="pagination-button" :class="{ 'active': page === currentPage }"
                                        @click="goToPage(page)">
                                        {{ page }}
                                    </button>
                                </span>
                                ...
                                <button class="pagination-button" @click="goToPage(totalPages)"
                                    :disabled="currentPage === totalPages">
                                    &raquo;
                                </button>
                            </div>
                        ...
                        <div class="modal-content large">
                            ...
                            <template v-else>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="detail-section">
                                        <h3>주문 상품</h3>
                                        <div v-for="item in selectedOrder.items" :key="item.orderItemNo"
                                            class="product-card">
                                            <div class="product-main-info">
                                                <div class="product-image">
                                                    <img :src="'${pageContext.request.contextPath}' + item.imageUrl"
                                                        alt="상품 이미지" v-if="item.imageUrl">
                                                    <span v-else>📦</span>
                                                </div>
                                                <div class="product-info">
                                                    <div class="product-info-name">{{ item.productName }}</div>
                                                    <div v-if="item.optionUnit" class="product-info-option">옵션: {{
                                                        item.optionUnit }} </div>
                                                    <div class="product-info-quantity">수량: {{ item.quantity }}개</div>
                                                    <div class="product-info-price">{{ item.price.toLocaleString() }}원
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div v-if="item.refundStatus" class="refund-details">
                                                <div class="refund-info">
                                                    <p><strong>환불 상태:</strong> <span
                                                            class="badge badge-refund-request">{{ item.refundStatus
                                                            }}</span></p>
                                                    ...
                                                    <p>
                                                        ...
                                                        <span>
                                                            ...
                                                            <span v-if="showShippingRefundHint(item)"
                                                                class="refund-hint">
                                                                (전체 환불 시 배송비 {{
                                                                selectedOrder.deliveryFee.toLocaleString() }}원 추가 환불 필요)
                                                            </span>
                                                        </span>
                                                    </p>
                                                ...
                                                <div v-if="item.refundStatus === '대기'" class="refund-actions">
                                                    <button class="btn btn-success btn-sm"
                                                        @click="processRefund(item, '승인')">승인</button>
                                                    <button class="btn btn-danger btn-sm"
                                                        @click="processRefund(item, '거절')">거절</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="detail-box">
                                            ...
                                            <div class="detail-row">
                                                ...
                                                <span class="detail-value">
                                                    {{ selectedOrder.items.reduce((acc, i) => acc + (i.price *
                                                    i.quantity), 0).toLocaleString() }}원
                                                </span>
                                            </div>
                                            ...
                                            <div class="detail-row">
                                                ...
                                                <span class="detail-value">{{ Number(selectedOrder.deliveryFee ||
                                                    0).toLocaleString() }}원</span>
                                            </div>

                                            <!-- 쿠폰 할인 -->
                                            <div class="detail-row" v-if="selectedOrder.couponDiscount > 0"
                                                style="color: #e11d48;">
                                                <span class="detail-label">쿠폰 할인</span>
                                                <span class="detail-value">-{{
                                                    Number(selectedOrder.couponDiscount).toLocaleString() }}원</span>
                                            </div>

                                            <!-- 환불액 -->
                                            <div v-if="getRefundedAmount(selectedOrder) > 0" class="detail-row"
                                                style="color: #dc3545;">
                                                <span class="detail-label">- 환불된 금액</span>
                                                <span class="detail-value">{{
                                                    getRefundedAmount(selectedOrder).toLocaleString() }}원</span>
                                            </div>
                                            ...
                                            <div class="detail-row total">
                                                ...
                                                <span class="detail-value">
                                                    ...
                                                    getRefundedAmount(selectedOrder)).toLocaleString() }}원
                                                </span>
                                            </div>
                                            <div class="detail-row">
                                                <span class="detail-label">결제수단</span>
                                                <span class="detail-value">{{ selectedOrder.paymentMethod || '정보 없음'
                                                    }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div v-if="selectedOrder.courier" class="detail-section">
                                        ...
                                        <div class="detail-box">
                                            ...
                                            <div class="detail-row">
                                                ...
                                                <span class="detail-value tracking-number-value">{{
                                                    selectedOrder.trackingNo}}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <script>
            // ✅ SweetAlert2 helper (요구사항: 성공만 confirm 버튼 색상 적용)
            ...
            const swConfirm = (title, text, confirmText = '확인', cancelText = '취소') => Swal.fire({
                icon: 'question',
                title: title || '',
                text: text || '',
            ...
            const app = Vue.createApp({
                ...
                methods: {
                    ...
                    async processRefund(item, newStatus) {
                        const actionText = newStatus === '승인' ? '승인' : '거절';

                        const ok = await swConfirm(
                            '환불 처리',
                            `"${item.productName}" 상품의 환불 요청을 ${actionText}하시겠습니까?`,
                            actionText,
                            '취소'
                        );
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.result === 'success') {
                                    swSuccess('완료', '환불 요청이 ' + actionText + ' 처리되었습니다.').then(() => {
                                        self.openDetailModal(self.selectedOrder);
                                    });
                                } else {
                                    swError('실패', response.message || '환불 처리 중 오류가 발생했습니다.');
                                }
                            },
                            error: function () {
                    ...
                    async applyBulkAction() {
                        ...
                        const ok = await swConfirm(
                            '일괄 상태 변경',
                            `${self.selectedOrders.length}개의 주문 상태를 '${self.bulkActionStatus}'(으)로 변경하시겠습니까?`,
                            '변경',
                            '취소'
                        );
                        if (!ok.isConfirmed) return;
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.result === "success") {
                                    swSuccess('완료', data.message || "주문 상태가 성공적으로 변경되었습니다.").then(() => {
                                        self.selectedOrders = [];
                                        self.fnLoadOrders();
                                    });
                                } else {
                                    swError('오류', data.message || '일괄 변경에 실패했습니다.');
                                }
                            },
                            error: function () {
                                swError('서버 오류', "서버와 통신 중 오류가 발생했습니다.");
                        ...
                        if ((currentStatus === '배송 준비중' || currentStatus === '결제완료' || currentStatus === '신규 주문') &&
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if (data.result === "success") {
                                        self.fnLoadOrders();
                                        swSuccess("완료", "주문 상태가 업데이트되었습니다.");
                                    } else {
                                        swError('실패', data.message || '주문 상태 업데이트에 실패했습니다.');
                                    }
                                },
                                error: function (xhr, status, error) {
                                    swError('서버 오류', '서버와 통신 중 오류가 발생했습니다. ' + (error || ''));
                                }
                            });
                        }
                    },
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.result === "success") {
                                    ...
                                    swSuccess("완료", "배송 정보가 등록되고 주문 상태가 '배송중'으로 변경되었습니다.").then(() => {
                                        self.fnLoadOrders();
                                    });
                                } else {
                                    swError('실패', data.message || '배송 정보 등록에 실패했습니다.');
                                }
                            },
                            error: function (xhr, status, error) {
                                swError('서버 오류', '서버와 통신 중 오류가 발생했습니다. ' + (error || ''));
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.result === "success" && data.order) {
                                    ...
                                    swError("실패", "주문 상세 정보를 불러오는데 실패했습니다.").then(() => {
                                        self.detailModalOpen = false;
                                    });
                                }
                            },
                            error: function () {
                                swError("서버 오류", "서버와 통신 중 오류가 발생했습니다.").then(() => {
                                    self.detailModalOpen = false;
                                });
                            }
                        });
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.result === "success") {
                                    ...
                                    swError('실패', data.message || '주문 목록을 불러오는데 실패했습니다.');
                                }
                            },
                            error: function (xhr, status, error) {
                                swError('서버 오류', '서버와 통신 중 오류가 발생했습니다. ' + (error || ''));
                    ...
                    getRefundedAmount(order) {
                        ...
                        if (isAllRefunded) {
                            if (order.couponDiscount > 0) refundTotal -= order.couponDiscount;
                            if (isBeforeShipping) refundTotal += 3000;
                        }
                    ...
                    getOrderOverallRefundStatus(order) {
                        if (!order || order.totalItemCount === undefined) return null;
                        ...
                        if (pendingRefundItems > 0) return '대기';
                        if (processedRefundItems > 0 && processedRefundItems === totalItems) return '전체 환불 완료';
                        if (processedRefundItems > 0 && processedRefundItems < totalItems) return '부분 환불 완료';
                        return null;
                    },
                },
                mounted() {
