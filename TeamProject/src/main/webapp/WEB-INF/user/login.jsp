                        ...
                        <div class="social-login">
                            <a href="/oauth2/authorization/kakao">
                                <button type="button" class="kakao">
                                    <img src="${pageContext.request.contextPath}/resources/img/login-img/btn_login_kakao.png"
                                        alt="카카오 로그인">
                                </button>
                            </a>
                            <a href="/oauth2/authorization/naver">
                                <button type="button" class="naver">
                                    <img src="${pageContext.request.contextPath}/resources/img/login-img/btn_login_naver.png"
                                        alt="네이버 로그인">
                                </button>
                            </a>
                            <a href="/oauth2/authorization/google">
                                <button type="button" class="google">
                                    <img src="${pageContext.request.contextPath}/resources/img/login-img/btn_login_google.png"
                                        alt="구글 로그인">
                                </button>
                            </a>
                        </div>
        ...
        const app = Vue.createApp({
            ...
            methods: {
                fnLogin: function () {
                    ...
                    if (!self.userId || !self.userPwd) {
                        Swal.fire('⚠️', '아이디와 비밀번호를 입력해주세요.', 'warning');
                        return;
                    }
                    let param = {
                        userId: self.userId,
                    ...
                    $.ajax({
                                            ...
                                            success: function (recoverResponse) {
                                                if (recoverResponse.result == "success") {
                                                    Swal.fire({
                                                        icon: 'success',
                                                        title: '✅',
                                                        text: '계정이 성공적으로 복구되었습니다. 다시 로그인해주세요.',
                                                        confirmButtonColor: '#5dbb63'
                                                    }).then(() => location.reload());
                                                } else {
                                                    Swal.fire('❌', (recoverResponse.msg || "계정 복구 중 오류가 발생했습니다."), 'error');
                                                }
                                            },
                                            error: function () {
                                                Swal.fire('❌', '계정 복구 서버와 통신 중 오류가 발생했습니다.', 'error');
                                            }
                                        });
                                    } else {
                                        Swal.fire('ℹ️', '계정 복구를 취소했습니다.', 'info');
                                    }
                                });
                            }
                            else {
                        ...
                        error: function () {
                            Swal.fire('❌', '로그인 서버와 통신 중 오류가 발생했습니다.', 'error');
                        }
                    });
                },
            },
            mounted() {
                ...
                if (urlParams.has('error') && urlParams.get('error') === 'true') {
                    ...
                    if (errorMessage) {
                        Swal.fire('❌', decodeURIComponent(errorMessage), 'error');
                    } else {
